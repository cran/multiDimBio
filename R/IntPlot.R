IntPlot <-
function(Scores, Cov.A, Cov.B, pvalues=rep(1,8), int.pvalues=rep(1,4)){
	...
	plot(rep(0,4),1:4,pch=15,col=colors, cex=5, xaxt='n',yaxt='n',xlab='',ylab='',xlim=c(0,0.5),ylim=c(0,4),lwd=2)
	...
	for(i in 1:4){
		interaction.plot(Cov.A,Cov.B,Scores[,i], legend=TRUE,ylab=paste('Axis',i,'Score',sep=' '),xlab='', col=c(COL[i,1],COL[i,2]), lty=LTY[i], lwd=2)
		}
	dev.off()
	...
	for(i in 1:4){
		interaction.plot(Cov.A,Cov.B,Scores[,i], legend=FALSE,ylab=paste('Axis',i,'Score',sep=' '),xlab='', col=c(COL[i,1],COL[i,2]), lty=LTY[i], lwd=2)
		}
	dev.off()
