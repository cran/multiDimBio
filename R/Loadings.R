Loadings <-
function(DATA,GROUPS,method=c('PCA','LDA')){
	
	if(sum(method=='PCA')>0){
		nPCS=floor(ncol(DATA)/5)
		
		PPCA<-pca(DATA, nPcs=nPCS,method='ppca',center=TRUE,scale='vector')
		...
		barplot(abs(OUT[,i]),main=paste(title,'- Variance Explained = ',round(PPCA@R2[i],3)),cex.names=0.5)
		dev.off()
		}#end for i
